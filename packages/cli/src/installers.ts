...
async function upsertHermesMemoryMaturationCron(input: {
  ...
  const prompt = [
    ...
    "A pre-run script asks Cristalina to mature consolidated runtime evidence into governed structured memory claims.",
    "If the script succeeds, it returns wakeAgent=false and this cron stays silent.",
    "If the script fails, report the script error concisely. Do not create cron jobs or edit Cristalina code.",
  ].join(" ");
...
export async function installRuntime(input: RuntimeInstallInput): Promise<RuntimeInstallResult> {
  ...
  const memoryMaturationScript = pluginPaths.memoryMaturationScriptPath
    ? [
        ...
        "export CRISTALINA_MEMORY_MATURATION_RUNTIME_MANAGED=\"${CRISTALINA_MEMORY_MATURATION_RUNTIME_MANAGED:-1}\"",
        `exec ${shellQuote(process.execPath)} ${shellQuote(cliPath)} memory mature --runtime ${shellQuote(input.runtime)} --write --config ${shellQuote(loaded.configPath)} --max-items "$MAX_ITEMS"`,
        "",
      ].join("\n")
    : null;
  ...
  const memoryMaturationCronScript = pluginPaths.memoryMaturationCronScriptPath
    ? [
        ...
        "timeout = int(os.environ.get('CRISTALINA_MEMORY_MATURATION_TIMEOUT_SECONDS', '180'))",
        `cmd = ${JSON.stringify([process.execPath, cliPath, "memory", "mature", "--runtime", input.runtime, "--write", "--config", loaded.configPath, "--max-items"])} + [max_items]`,
        "env = os.environ.copy()",
        "env.setdefault('CRISTALINA_MEMORY_MATURATION_RUNTIME_MANAGED', '1')",
