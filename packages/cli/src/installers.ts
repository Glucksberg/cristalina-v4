...
export async function installRuntime(input: RuntimeInstallInput): Promise<RuntimeInstallResult> {
  ...
  const memoryCycleCronScript = pluginPaths.memoryCycleCronScriptPath
    ? [
        ...
        "        f\"- owner decisions: backlog {display(status.get('owner_decisions_backlog', owner.get('owner_decision_count')))}, in review {display(status.get('owner_decisions_in_review'))}, blocking {display(status.get('owner_decisions_blocking'))}; pending reviews hermes/openclaw: {display(status.get('pending_hermes'))}/{display(status.get('pending_openclaw'))}\",",
        "        \"\",",
        "        \"Files:\",",
        ...
        "        owner_decisions = status_json.get('owner_decisions') if isinstance(status_json, dict) else {}",
        "        payload['status_summary'] = {'pending_hermes': nested(pending or {}, 'hermes'), 'pending_openclaw': nested(pending or {}, 'openclaw'), 'owner_decisions_backlog': nested(owner_decisions or {}, 'backlog'), 'owner_decisions_in_review': nested(owner_decisions or {}, 'in_review'), 'owner_decisions_blocking': nested(owner_decisions or {}, 'blocking')}",
