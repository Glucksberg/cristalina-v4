...
function buildHermesMemoryProviderEntrypoint(): string {
  return [
    ...
    "",
    "    def _compile_context(self, query: str) -> str:",
    ...
    "        if result:",
    "            return result",
    "        return self._compile_context(query)",
    "",
    ...
    "        def _run() -> None:",
    "            try:",
    "                result = self._compile_context(query)",
    "                with self._prefetch_lock:",
    ...
    "        return self._compile_context('session compression continuity')",
    "",
    ...
    "            return json.dumps({'context': self._compile_context(query)}, ensure_ascii=False)",
