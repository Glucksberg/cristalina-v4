...
test("CLI parser recognizes the planned command surface", () => {
  ...
  assert.deepEqual(parseCristalinaCommand(["memory", "mature", "--runtime", "hermes", "--write", "--max-items", "12", "--llm-output", "maturation.json"]), {
    ...
    evidenceOutputPath: undefined,
  });
  assert.deepEqual(parseCristalinaCommand(["memory", "candidates", "--runtime", "hermes", "--limit", "25"]), {
    name: "memory",
    action: "candidates",
    configPath: undefined,
    storeRoot: undefined,
  ...
  assert.deepEqual(parseCristalinaCommand(["reviews", "list", "--owner-decisions"]), {
    name: "reviews",
    action: "list",
    configPath: undefined,
    storeRoot: undefined,
