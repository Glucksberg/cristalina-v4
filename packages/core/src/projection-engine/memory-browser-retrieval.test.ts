...
test("memory browser inspects symbolic and vector retrieval artifacts without changing authority", () => {
  ...
  assert.equal(snapshot.consistency.observed_layer_updates.canon, "2026-04-21T00:00:00.000Z");
  assert.equal(snapshot.consistency.observed_layer_updates.derived, "2026-04-21T00:00:00.000Z");
  assert.equal(browser.manifest.boundary_note, snapshot.consistency.boundary_note);
  assert.equal(browser.manifest.observed_layer_updates?.canon, snapshot.consistency.observed_layer_updates.canon);
  assert.equal(browser.manifest.observed_layer_updates?.derived, snapshot.consistency.observed_layer_updates.derived);
