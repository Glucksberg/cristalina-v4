...
import { buildConversationPreferenceFlowInput } from "../../core/dist/test-support/conversation-preference-fixtures.js";
...
test("OpenClaw adapter exposes pending owner review items from the latest projection", async (t) => {
  ...
  const result = await writeConversationPreferenceFlowToStore(buildConversationPreferenceFlowInput({
    rootDir,
...
test("OpenClaw adapter exposes closed owner review items after queue ratification", async (t) => {
  ...
  await ratifyQueuedConversationPreferenceProposalToStore({
    ...
    actor: "owner:openclaw-adapter-test",
